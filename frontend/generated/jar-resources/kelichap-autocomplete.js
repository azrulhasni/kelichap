...
import '@vaadin/icons';
import './kelichap-autocomplete-overlay';

/**
 * `<kelichap-autocomplete>` Web Component with a text input that provides a panel of suggested options.
 *
 * ```html
 * <kelichap-autocomplete></kelichap-autocomplete>
 * ```
 *
 * @memberof Vaadin
 * @mixes ElementMixin
...
registerStyles(
        'vaadin-item',
        css`
        :host(.kelichap-autocomplete-no-checkmark) [part="checkmark"] {
            visibility: hidden;
        }
    `
        );
class KelichapAutocomplete extends ElementMixin(ThemableMixin(PolylitMixin(LitElement))) {
    ...
    render() {
        return html`
            ...
            <kelichap-autocomplete-overlay .opened="${this.opened}" theme="${this._theme}">
                <vaadin-list-box part="options-container" theme="${this._theme}">
                    ${!this.loading ? html`
                        ${map(this._limitedOptions, (option) => html`
                            <vaadin-item @click="${this._optionClicked}" part="option" theme="${this._theme}" class="kelichap-autocomplete-no-checkmark">
    ...
    firstUpdated() {
        ...
        this._overlayElement = this.shadowRoot.querySelector('kelichap-autocomplete-overlay');
    ...
    _setOverlayPosition() {
        ...
        this._overlayElement.updateStyles({'--kelichap-autocomplete-options-width': inputRect.width + 'px'});
    }
...
//    }
    ...
    _applyValue(value) {
        this.dispatchEvent(
                new CustomEvent('kelichap-autocomplete-value-applied', {
                    bubbles: true,
                    detail: {
                        value: value
                    }
    ...
    _onKeyUp(event) {
        if (/[\p{L}\p{N}]/gu.test(String.fromCharCode(event.keyCode))) { //match letters and numbers unicode character
            ...
            this.dispatchEvent(
                    new CustomEvent('kelichap-autocomplete-value-token', {
                        bubbles: true,
                        detail: {
                            token: token
                        }
...
customElements.define(KelichapAutocomplete.is,KelichapAutocomplete);

/**
 * @namespace Vaadin
 */
window.Vaadin.KelichapAutocomplete = KelichapAutocomplete;